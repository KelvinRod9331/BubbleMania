...
function getUserScore(req, res, next) {
  ...
  db
    .one("select current_score from scores where username = ${username}", req.user)   //look over this line
    .then(function(data) {
      console.log("got current_score: ", data);
      res.status(200).json({
        status: "success",
        current_score: data.current_score,
        message: "Fetched user current_score"
      });
    })
    .catch(function(err) {
      return next(err);
